...
[link](https://www.realclearpolitics.com/epolls/2016/president/us/general_election_trump_vs_clinton-5491.html)
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/standard-error-versus-sample-size-1.png)
...
In summary, we have that ¯X has an approximately normal distribution with expected value p and standard error sqrt(p(1-p)/N).
...
Now we continue with our calculation, but dividing by SE^(¯X)=sqrt(¯X(1-¯X)/N) instead. In our first sample we had 12 blue and 13 red so ¯X=.48 and our estimate of standard error is:
```{r}
x_hat<-0.48
...
To review, the theory tells us that ¯X is approximately normally distributed, has expected value p=0.45 and standard error sqrt(p(1-p)/N)=0.016. The simulation confirms this:
```{r}
mean(x_hat)
sd(x_hat)
```
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/normal-approximation-for-polls-1.png)
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/standard-error-versus-p-1.png)
...
```{r}
B<-10000
errors<-replicate(B, take_sample(0.45, 100))-p
```
...
set.seed(1)
B<-10000
x<-replicate(B, take_sample(0.45, 100))
sd(x)
```
...
c. approximately normal with expected value ¯X and standard error sqrt(¯X(1-¯X)/N).
d. nor a random variable.
...
It may follow a normal distribution.
