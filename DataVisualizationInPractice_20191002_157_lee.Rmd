...
```{r}
filter(gapminder, year==1962)%>%ggplot(aes(fertility, life_expectancy, col=continent))+geom_point()
```
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/facet-without-fixed-scales-1.png)
...
gapminder%>%filter(country%in%countries&!is.na(fertility))%>%ggplot(aes(year,fertility))+geom_line()
```
...
```{r}
countries <- c("South Korea","Germany")
gapminder%>%filter(country%in%countries&!is.na(fertility))%>%ggplot(aes(year, fertility, group = country, col=country))+geom_line()
```
...
e use the color = "black" argument to draw a boundary and clearly distinguish the bins.
...
```{r}
gapminder%>%filter(year==past_year&!is.na(gdp))%>%ggplot(aes(log2(dollars_per_day)))+geom_histogram(binwidth = 1,col='black')
```
...
```{r}
gapminder%>%filter(year==past_year)%>%ggplot(aes(log10(population)))+geom_histogram(binwidth = 0.5, col='black')
```
...
```{r}
gapminder%>%filter(year==past_year&!is.na(gdp))%>%ggplot(aes(dollars_per_day))+geom_histogram(binwidth = 1,col='black')+scale_x_continuous(trans='log2')
```
...
```{r}
p+geom_point()+theme(axis.text.x=element_text(angle=90, hjust=1))
```
...
```{r}
p<-gapminder%>%filter(year==past_year&!is.na(gdp))%>%mutate(region=reorder(region,dollars_per_day,FUN=median))%>%ggplot(aes(region,dollars_per_day))+geom_point()+theme(axis.text.x=element_text(angle=90, hjust=1)) 
p
```
...
```{r}
p<-gapminder%>%filter(year == past_year & !is.na(gdp))%>%ggplot(aes(group, dollars_per_day))+geom_boxplot()+theme(axis.text.x = element_text(angle = 90, hjust = 1))+scale_y_continuous(trans = "log2")+xlab("")
p 
```
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/boxplot-dont-show-bimodal-1.png)
...
```{r}
p+geom_density_ridges(jittered_points=T, position = position_points_jitter(width = 0.05, height = 0), point_shape = '|', point_size = 3, point_alpha = 1, alpha = 0.7)
```
...
past_year=1970
present_year=2010
gapminder%>%filter(year%in%c(past_year, present_year)&!is.na(gdp))%>%mutate(west=ifelse(group=='West','West','Developiing'))%>%ggplot(aes(dollars_per_day))+geom_histogram(binwidth=1,col='black')+scale_x_continuous(trans='log2')+facet_grid(year~west)
```
...
```{r}
gapminder%>%filter(year%in%c(past_year, present_year)&!is.na(gdp)&country%in%country_list)%>%mutate(west=ifelse(group=='West','West','Developiing'))%>%ggplot(aes(dollars_per_day))+geom_histogram(binwidth=1,col='black')+scale_x_continuous(trans='log2')+facet_grid(year~west)
```
...
```{r}
gapminder%>%filter(year%in%c(past_year, present_year)&country%in%country_list)%>%ggplot(aes(group, dollars_per_day))+geom_boxplot()+theme(axis.text.x=element_text(angle=90, hjust=1))+scale_y_continuous(trans='log2')+xlab("")+facet_grid(.~year)
```
...
```{r}
gapminder%>%filter(year%in%c(past_year, present_year)&country%in%country_list)%>%mutate(year=factor(year))%>%ggplot(aes(group, dollars_per_day, fill=year))+geom_boxplot()+theme(axis.text.x=element_text(angle=90, hjust=1))+scale_y_continuous(trans='log2')+xlab("")
```
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/income-west-v-devel-before-after-ratio-1.png)
...
```{r}
gapminder%>%filter(year %in% c(past_year, present_year) & country %in% country_list)%>%ggplot(aes(dollars_per_day))+geom_density(fill='grey')+scale_x_continuous(trans='log2')+facet_grid(.~year)
```
...
```{r}
head(gapminder)
table(ifelse(gapminder$group=='West', 'West', 'Developing'))
#? 어디서 저 교재 숫자가 나왔더라 기억이안나네
```
...
```{r}
gapminder%>%filter(year%in%c(past_year, present_year)&country%in%country_list)%>%mutate(group=ifelse(group=='West', 'West', 'Developing'))%>%ggplot(aes(dollars_per_day, fill=group))+scale_x_continuous(trans='log2')+geom_density(alpha=0.2)+facet_grid(year~.)
```
...
```{r}
p<-gapminder%>%filter(year%in%c(past_year, present_year)&country%in%country_list)%>%mutate(group=ifelse(group=='West', 'West', 'Developing'))%>%ggplot(aes(dollars_per_day, y=..count..,fill='group'))+scale_x_continuous(trans='log2', limit=c(0.125, 300))
...
```{r}
gapminder%>%filter(year%in%c(past_year, present_year)&country%in%country_list)%>%ggplot(aes(dollars_per_day, group))+scale_x_continuous(trans='log2')+geom_density_ridges(adjust=1.5)+facet_grid(.~year)
```

Another way to achieve this is by stacking the densities on top of each other:
```{r}
gapminder%>%filter(year%in%c(past_year, present_year)&country%in%country_list)%>%group_by(year)%>%mutate(weight=population/sum(population)*2)%>%ungroup()%>%ggplot(aes(dollars_per_day, fill=group))+scale_x_continuous(trans='log2')+geom_density(alpha=0.2, bw=0.75, position='stack')+facet_grid(year~.)
```
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/income-smooth-density-counts-by-region-year-weighted-1.png)
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/ecological-fallacy-averages-1.png)
...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/ecological-fallacy-all-data-1.png)
