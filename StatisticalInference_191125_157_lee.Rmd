---
title: "Ch 15 Statistical inference"
...
So, for example, for p=0.95, 1-(1-p)/2=0.975 and we get the 1.96 we have been useing:
```{r}
qnorm(0.975)
```
...
When using the theory we described above, it is important to remember that it is the intervals that are random, not p. In the plot above, we can see the random intervals moving around and p, represented with the vertical ling, staying in the same place. The proportion of blue in the urn p is not. So the 95% relates to the probability thal this random interval falls on top of p. Saying the p has a 95% chance of being between this and that is technically an incorrect statement because p is not random.
...
```{r}
N<-polls$samplesize
polls$x_hat<-polls$rawpoll_clinton/100
polls$se_hat<-sqrt(x_hat*(1-x_hat)/N)
...
```{r}
p=0.482
poll$hit<-ifelse(poll$lower<=p & poll$upper>=p, 1, 0)
poll
```
...
```{r}
N<-polls$samplesize
polls$x_hat<-(polls$d_hat+1)/2
polls$se_hat<-sqrt(x_hat*(1-x_hat)/N)
polls$lower<-polls$x_hat-1.96*polls$se_hat
polls$upper<-polls$x_hat+1.96*polls$se_hat
p=0.482
polls$hit<-ifelse(polls$lower<=p & polls$upper>=p, 1, 0)
polls%>%select(pollster, enddate, x_hat, lower, upper, hit)
```
...
```{r}
polls$err<-polls$d_hat-0.021
polls%>%
  group_by(pollster)%>%
  ggplot(aes(pollster, err))+
  geom_boxplot()+
...
```{r}
p1<-as.data.frame(table(polls$pollster))
polls_prime<-p1%>%
  filter(Freq>=5)
polls%>%
  filter(pollster%in%polls_prime$Var1)%>%
  group_by(pollster)%>%
  ggplot(aes(pollster, err))+
  geom_boxplot()+
...
Let’s consider the blue and red beads. Suppose that rather than wanting an estimate of the spread or the proportion of blue, I am interested only in the question: are there more blue beads or red beads? I want to know if the sprep-values are ubiquitous in the scientific literature. They are related to confidence intervals so we introduce the concept here.
...
Pr(sqrt(N)(|¯X-0.5|)/sqrt(0.5(1-0.5)) > sqrt(N)(0.02-0.5)/sqrt(0.5(1-0.5)))
...
```{r}
data.frame(var = c('Awarded', 'Not Awarded'),
           Men = c('a','c'),
           Women = c('b','d'))
```
...
log(ad/bc)±1.96sqrt(1/a + 1/b + 1/c + 1/d)
...
Note that the log odds ratio is not defined if any of the cells of the two-by-two table is 0. This is because if a, b, c, or d is 0, the log(ad/bc) is is either the log of 0 or has a 0 in the denominator. For this situation, it is common practice to avoid 0s by adding 0.5 to each cell. This is referred to as the Haldane–Anscombe correction and has been shown, both in practice and theory, to work well.

## 15.10.7 Large sampless, small p-values
...
```{r}
ath<-data.frame(result=c('winning','losing'),
                career=c(500*0.7,500*(1-0.7)),
                events=c(8, 9))
ath_chi<-ath%>%select(-result)%>%chisq.test()
ath_chi$p.value
```
...
```{r}
car_odds<-with(ath,(career[2]/sum(career))/(career[1]/sum(career)))
pre_odds<-with(ath,(events[2]/sum(events))/(events[1]/sum(events)))
log_or <- log(pre_odds / car_odds)
se <- ath%>%select(-result) %>%
  summarize(se = sqrt(sum(1/events) + sum(1/career))) %>%
  pull(se)
ci <- log_or + c(-1,1) * qnorm(0.975) * se
pre_odds / car_odds
exp(ci)
...
```{r}
ath<-ath%>%mutate(career=career*2, events=events*2)
ath_chi<-ath%>%select(-result)%>%chisq.test()
ath_chi$p.value
```
