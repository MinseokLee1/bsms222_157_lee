...
```{r}
murders %>% select(state, rate) %>% filter(rate <= 0.71)
```
...
is the first argument of the select function, and the new data frame (formerly new_table) is the first argument of the filter function.
...
```
In the solution to the provious exercise, we did the following:
```{r}
my_states <- filter(murders, region %in% c("Northeast", "West") & rate < 1)
select(my_states, state, rate, rank)
```
...
```{r}
my_states <- murders%>%mutate SOMETHING%>%filter SOMETHING%>%select SOMETHING
```

```{r}
my_states <- murders%>%filter(region %in% c("Northeast", "West"))%>%filter(rate<=1)%>%select(state, rate, rank)
my_states
```
