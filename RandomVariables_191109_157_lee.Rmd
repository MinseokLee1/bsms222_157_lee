...
[link](https://rafalab.github.io/dsbook/book_files/figure-html/normal-approximates-distribution-1.png)
...
```{r}
B<-10^6
x<-sample(c(-1,1), B, replace=T, prob=c(9/19, 10/19))
mean(x)
```

In general, if the urn has two possible outcomes, say a and b, with proprotions p and 1-p respectively, the average is:

E[X]=ap+b(1-p)

Th see this, notive that if there are n beads in the urn, then we have 'np a's and 'n(1-p)b's and because the average is the sum, n * a * p + n * b * (1-p), divided by the total n, we get that the average is ap+b(1-p).
...
So in our roulette example, the sd of the values inside the urn is:
|1-(-1)|sqrt(10/19 X 9/19) or:
```{r}
2*sqrt(90)/19
```
...
This is because the sd function R does not return the sd of the list, but rather uses a formula that estimates standard deviations of a population from a random sample X1, ..., X_N which, for reasons not discussed here, divide the sum of squares by the N-1.

X`=(1/N)∑[1,N](Xi)
s=sqrt((1/(N-1))∑[1,N](Xi-X`)^2)
