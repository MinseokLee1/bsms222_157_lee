...
which is another way of writing the result we show above for the sum of darws.
...
set.seed(1)
n<-1000
S<-sample(c(17,-1), n, prob=c(1/19, 18/19), replace=T)
sum(S)
```
...
n<-1000
B<-1000
winning_money<-function(n){
  S<-sample(c(17,-1), n, prob=c(1/19, 18/19), replace=T)
  sum(S)
}
x<-replicate(B, winning_money(n))
mean(x)
sd(x)
```
...
```{r}
n<-1000
Y<-sample(c(1,-1), n, prob=c(1/19, 18/19), replace=T)
```
...
SE[(X1+...+X1)/n]=SE[nX1/n]=SE[X1]=σ>σ/sqrt(n)
...
n<-10000
p<-0.3
earn_per_foreclosure<- 200000
defaults<-sample(c(0,1),n,prob=c(1-p,p),replace=T)
S<-sum(defaults*earn_per_foreclosure)
```
...
earn<-replicate(B, {
  defaults<-sample(c(0,1),n,prob=c(1-p,p),replace=T)
  sum(defaults*earn_per_foreclosure)
})
data.frame(earn)%>%ggplot(aes(earn))+geom_histogram(color='black')
```

3. What is the expected value of S?

```{r}
#E[S]={lp+x(1-p)}n above.
(200000*0.3+0*0.7)*n
```
...
```{r}
r<-(earn_per_foreclosure*p/(1-p))/180000
r
```

6. (Harder) What should the interest rate be so that the chance of losing money is 1 in 20? In math notation, what should the interest rate be so that Ps(S<0)=0.05?

```{r}
e<-earn_per_foreclosure
x<-r*180000
z<-qnorm(0.05)
n<-ceiling((z^2*e^2*p*(1-p))/(e*p+x*(1-p))^2)
n
```
