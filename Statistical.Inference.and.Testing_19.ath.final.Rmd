...
지금까지 위에서 우리는 정규성을 대충 보고 정규분포라고 가정해서 신뢰 구간을 구했는데, 사실 N=5 정도에서는 정규분포라 간다고 하기가 힘들 수 있어요. 이때는 아실지 모르겠지만 그래서 우리가 모분산을 모르기 때문에 t분포를 사용해야 맞아요. 분포를 사용한다는 말은, 임계값을 가져오는 분포 표로 해당 분포의 표를 쓴다는 거예요. 코드짤때도 그래서 엄밀하게는 'pnorm' 대신 'pt'를 사용해야 맞습니다.
