...
2. w create a vector with the city names and call the object city.
...
```
So there are 86 numbers in the list.
...
states[51]
```
So the name of the state with the smallest population is "Wyoming".
...
sum(ind)
##In _sum_, logical true values are regarded as one, false values as zero.
```
...
### Professor, Could you give me a feedback for this question..?
...
1+1/2^2+1/3^2+…1/100^2? Hint: thanks to Euler, we know it should be close to π^2/6.
...
```
There are 27 states below the average.
...
```{r}
pops <- log10(pop_in_millions)
total <- log10(total_gun_murders)
plot(pops, total)
```
