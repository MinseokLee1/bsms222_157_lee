...
# 12.3 The inter quarile range (IQR)
...
In R, points falling outside the whiskers of the boxplot are referred to as outliers. This definition of outlier was introduced by Tukey. The top whisker ends at the 75th percentile plus 1.5xIQR. Similarly the bottom whisker ends at the 25th percentile minus 1.5xIQR. If we define the first and third quartiles as Q1 and Q3 respectively, then an outlier is anything outside the range:
...
the average is '68.1', and the median is '68.2'.
...
# 12.7 Case study : self-feported student heights
...
```{r}
max_height<-quantile(reported_heights$height, 0.75)+3*IQR(reported_heights$height)
min_height<-quantile(reported_heights$height, 0.25)-3*IQR(reported_heights$height)
c(max_height, min_height)
```

```{r}
reported_heights%>% 
  filter(!between(height, min_height, max_height))%>% 
  select(orginal_heights)%>%
  head(n=10)
```
