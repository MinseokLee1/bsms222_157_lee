...
```{r}
NHANES%>%filter(AgeDecade == " 20-29")%>%summarize(min(BPSysAve, na.rm=TRUE), max(BPSysAve, na.rm=TRUE))
```
...
```{r}
fml <- NHANES%>%filter(Gender == "female")%>%group_by(AgeDecade)
summarize(fml, average = mean(BPSysAve, na.rm=TRUE), standard_deviation = sd(BPSysAve, na.rm=TRUE))
```

5. Repeat exercise 4 for males.

```{r}
ml <- NHANES%>%filter(Gender == "male")%>%group_by(AgeDecade)
summarize(ml, average = mean(BPSysAve, na.rm=TRUE), standard_deviation = sd(BPSysAve, na.rm=TRUE))
```
...
```{r}
NHANES%>%filter(Gender == 'male', AgeDecade == ' 40-49')%>%group_by(Race1)%>%arrange(BPSysAve)
NHANES%>%filter(Gender == 'male', AgeDecade == ' 40-49')%>%group_by(Race1)%>%arrange(desc(BPSysAve))
...
```{r}
grade <- data_frame(names = c("John", "Juan", "Jean", "Yao"),
                    exam1 = c(95, 80, 90, 85),
                    exam2 = c(90, 85, 85, 90))
grade
```
...
heights%>%
  filter(sex == "Female")%>%
  summarize(range = quantile(heights, c(0, 0.5, 1)))
```
...
```{r}
murders_tibble%>%select(population)%>%pull()%>%log()%>%mean()%>%exp()
```
...
s_n_2 = pull(map_df(n, cs_n2))
map_df(n, cs_n)%>%mutate(s_n_2)
```
